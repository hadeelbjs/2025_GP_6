    ...
    try {
        ...
        if (deviceName) {
            if (!user.registrationDevice) {
                await User.findByIdAndUpdate(userId, { registrationDevice: deviceName });
                console.log(`📱 registrationDevice محفوظ لأول مرة: ${deviceName}`);
            } else if (user.registrationDevice !== deviceName) {
                anomalies.push({
                    type: 'unknown_device',
                    detail: `تسجيل دخول من جهاز جديد: ${deviceName}`,
                });
                console.log(`🚨 unknown_device: ${deviceName}`);
            } else {
                console.log(`✅ الجهاز معروف: ${deviceName}`);
            }
        }
