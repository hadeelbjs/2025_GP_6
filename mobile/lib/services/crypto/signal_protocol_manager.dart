...
class SignalProtocolManager {
  ...
  // فك التشفير
  // ===================================
  Future<String?> decryptMessage(String senderId, int type, String body) async {
    try {
  ...
  Future<int?> _getLocalKeysVersion() async {
    final versionStr = await _storage.read(key: 'keys_version_$_currentUserId');
  ...
  Future<DecryptionResult> decryptMessageSafe(
    ...
    try {
      ...
      if (plaintext != null) {
        return DecryptionResult(success: true, message: plaintext);
      }
      ...
      if (errorStr.contains('DuplicateMessageException')) {
        print(' Duplicate message detected from $senderId, ignoring.');
        return DecryptionResult(
          success: false,
          needsKeySync: false,
      ...
      if (errorStr.contains('InvalidKeyException') ||
        ...
        print(' Session corruption detected from $senderId');
        ...
        return DecryptionResult(
          ...
          needsKeySync: true,
          error:
              'Session reset. Please resend message. '
  ...
  Future<DecryptionResult> handleDecryptionFailure(
    ...
    try {
      ...
      return DecryptionResult(success: false, error: 'فشل معالجة خطأ التشفير');
    }
  }
...
class SessionResetResult {
  ...
  SessionResetResult({required this.success, this.message, this.error});
}
